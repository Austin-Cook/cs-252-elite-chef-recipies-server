...
from settings import MESSAGE, RETRIEVED_FROM

# SUCCESS

def response_success_find(recipe: dict, retrieved_from: str):
    recipe[MESSAGE] = "Success: Found a matching recipe in " + retrieved_from
    recipe[RETRIEVED_FROM] = retrieved_from
    return jsonify(recipe), 200

def response_success_delete(num_deleted: int) -> Tuple[Response, int]:
    return _json_success("Deleted " + str(num_deleted) + " recipies from MongoDB Atlas"), 200
    
def response_success_add_to_mongo() -> Tuple[Response, int]:
    return _json_success("Recipe saved to MongoDB"), 200

# ERROR

def response_error_missing_query_param(param: str):
    return _json_error("Request missing query param: " + param), 400

def response_error_no_matching_item(item: str) -> Tuple[Response, int]:
    return _json_error("No matching item in Redis cache or MongoDB: " + item), 404

def response_error_unable_to_add_item_to_mongo() -> Tuple[Response, int]:
    return _json_error("Internal server error: Unable to add recipe to MongoDB Atlas"), 500

def response_error_not_json() -> Tuple[Response, int]:
    return _json_error("Request content was not JSON"), 400

def response_error_field_missing(field: str) -> Tuple[Response, int]:
    return _json_error("Missing field in request: " + field), 400

def response_error_field_empty(field: str) -> Tuple[Response, int]:
    return _json_error("Empty field in request: " + field), 400

def response_error_field_too_large(field: str, maxSize: int, actualSize: int) -> Tuple[Response, int]:
    return _json_error("Field too large: " + field + " " + str(actualSize) + "/" + str(maxSize) + " characters"), 400

def response_error_wrong_type(field: str) -> Tuple[Response, int]:
